...
class ReportGenerator:
    ...
    def _build_report_prompt(self, topic: str, paper_info: Dict[str, Any]) -> str:
...
"""
        ...
        prompt = f"""你是一位资深的学术研究专家，擅长撰写高质量的学术报告。请根据以下推荐的学术论文，撰写一份关于"{topic}"的综合学术报告。
...
## 摘要
简要概述该研究领域的背景、现状、主要发现和本报告的目的。(150-200字)
...
请直接输出完整的学术报告，无需额外说明。"""
    ...
    def _validate_citations(self, report_content: str, paper_info: Dict[str, Any]) -> Dict[str, Any]:
        papers_list = paper_info["papers"]
        num_papers = len(papers_list)

        citation_pattern = re.compile(r'\[(\d+)\]')
        citations = citation_pattern.findall(report_content)
        ...
        for cit in citations:
            cit_num = int(cit)
            if 1 <= cit_num <= num_papers:
                valid_citations.append(cit_num)
            else:
                invalid_citations.append(cit_num)

        unique_citations = list(set(valid_citations))

        return {
            "total_citations": len(citations),
            "valid_citations": len(valid_citations),
            "invalid_citations": len(invalid_citations),
            "unique_cited_papers": len(unique_citations),
            "cited_paper_indices": sorted(unique_citations)
        }
    ...
    def generate_report(
        ...
        if not search_results:
            return {
                "success": False,
                "error": "No search results provided to generate report"
            }
    ...
    def save_report_to_file(self, report_result: Dict[str, Any], output_dir: str = "./reports") -> str:
        if not report_result["success"]:
            raise ValueError("Cannot save failed report")

        os.makedirs(output_dir, exist_ok=True)

        topic_slug = re.sub(r'[^\w\-_\. ]', '_', report_result["topic"])
        ...
        with open(filepath, "w", encoding="utf-8") as f:
            f.write(report_result["report_content"])
...
def main():
    ...
    try:
        ...
        print("\n🚀 Generating report (note: requires valid API key)...")
        try:
            result = generator.generate_report(test_topic, test_results)
            if result["success"]:
                print("✅ Report generated successfully!")
                print(f"   Total papers used: {result['paper_info']['total_papers']}")
                print(f"   Total chunks used: {result['paper_info']['total_chunks']}")
                print(f"   Citation stats: {result['citation_stats']}")
                preview = result["report_content"][:500] + "..." if len(result["report_content"]) > 500 else result["report_content"]
                print(f"\n📄 Preview:\n{preview}")

                print("\n💾 Testing report saving...")
                try:
                    saved_path = generator.save_report_to_file(result)
                    print(f"✅ Report saved to: {saved_path}")
                except Exception as e:
                    print(f"⚠️  Report saving test skipped: {type(e).__name__}")
            else:
                print(f"❌ Failed: {result['error']}")
        except Exception as e:
            print(f"⚠️  Report generation test skipped: {type(e).__name__}")

        print("\nReport generator module test completed!")
    except Exception as e:
